...
init(component){super.init(component);this.sharedMemory=this.#getSharedMemory();this.restrictedKeys=this.#getRestrictedKeys();this.observers=this.#getObservers();}
...
#getObservers(){if(fbr._fabrSharedMemoryObservers){return fbr._fabrSharedMemoryObservers;}
fbr._fabrSharedMemoryObservers=new Map();return fbr._fabrSharedMemoryObservers;}
...
return false;}
getKeys(){return Object.keys(this.sharedMemory);}
...
return false;}
connect(variableName,observerComponent,callback){if(!this.observers.has(variableName)){this.observers.set(variableName,[]);}
this.observers.get(variableName).push({component:observerComponent,callback:callback,});}
update(variableName,value){if(this.sharedMemory[variableName]!==value){this.sharedMemory[variableName]=value;this.notifyObservers(variableName,value);}}
notifyObservers(variableName,value){const observers=this.observers.get(variableName);if(observers){for(const observer of observers){observer.callback.call(observer.component,value);}}}};fbr.FabrHelperWatcher=class extends fbr.FabrHelper{constructor(){super();this.helperName="Watcher";this.watchers={};}
...
resetNavChildren(nav,navChild){const navChildren=nav.querySelectorAll("li");navChildren.forEach((child)=>{if(child!==navChild){child.classList.remove(nav.activeClass);}});}};fbr.FabrIf=class extends fbr.FabrCoreComponent{constructor(){super();this.componentName="FabrIf";this.componentStyleClass="fabr-if";this.selector="[fabr-if]";this.eventMap={};this.observedVariables=new Map();this.sharedMemoryHelper=new fbr.FabrHelperSharedMemory();this.sharedMemoryHelper.init(this);}
init(){super.init();this.processElements();}
processElements(){for(const element of this.elements){const variableName=element.getAttribute("fabr-if");const target=element;if(!variableName){continue;}
this.observeVariable(variableName);this.observedVariables.set(variableName,{target,parent:target.parentNode,});this.updateTargetVisibility(variableName);}}
observeVariable(variableName){console.log(this.sharedMemoryHelper.getKeys());this.sharedMemoryHelper.connect(variableName,this,(_,newValue)=>{this.updateTargetVisibility(variableName,newValue);});this.observedVariables.set(variableName,[]);}
updateTargetVisibility(variableName,newValue){const targetData=this.observedVariables.get(variableName);if(newValue===undefined){newValue=this.sharedMemoryHelper.get(variableName);}
const{target,parent}=targetData;if(newValue===true){if(!parent.contains(target)){parent.appendChild(target);}}else{if(parent.contains(target)){parent.removeChild(target);}}}};fbr.Fabr=class extends fbr.FabrCore{constructor(){super();this.version="0.1.0";console.log(`%cFabr[${this.version}]initializing...`,"color: #00f");this.components={form:new fbr.FabrForm(),link:new fbr.FabrLink(),list:new fbr.FabrList(),carousel:new fbr.FabrCarousel(),contextmenu:new fbr.FabrContextMenu(),counter:new fbr.FabrCounter(),expander:new fbr.FabrExpander(),icon:new fbr.FabrIcon(),if:new fbr.FabrIf(),image:new fbr.FabrImage(),loader:new fbr.FabrLoader(),tooltip:new fbr.FabrTooltip(),nav:new fbr.FabrNav(),notebook:new fbr.FabrNotebook(),table:new fbr.FabrTable(),animator:new fbr.FabrAnimator(),preload:new fbr.FabrPreLoad(),snippet:new fbr.FabrSnippet(),selector:new fbr.FabrSelector(),summary:new fbr.FabrSummary(),};console.log("%cFabr initialized","color: #00f");}
